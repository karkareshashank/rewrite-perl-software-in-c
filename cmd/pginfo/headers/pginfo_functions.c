...
void pginfo_display_function(pginfo_options* pgopt)
{
	...
	else
	{
		...
		while(temp != NULL)
		{
			if(temp->pg_id > max_pgid)
				max_pgid = temp->pg_id;
	
			if(strlen(temp->relationship)  > rel_max_len)
				rel_max_len = strlen(temp->relationship);
