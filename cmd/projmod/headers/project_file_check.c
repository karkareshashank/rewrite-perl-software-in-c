...
void project_file_checker(char* filename)
{
	...
	while(!feof(file_ptr))
	{
		...
		{
			printf("projmod : Parse error on line %d , Incorrect number of fields , Should have 5 \":\"'s.\n",line_count);
			exit_flag = 1;
		}
		...
		{
			printf("projmod: Parse error on line %d, Invalid project name \"%s\", contains invalid characters\n",line_count,proj_detail_entry);
			exit_flag == 1;
		}
		...
		{
			printf("projmod: Parse error on line %d , Invalid project ID \"\"\n",line_count);
			exit_flag = 1;
		}
		...
		if(error_user != NULL)
		{
			printf("projmod : Parse error on line %d , Invalid user \"%s\"\n",line_count,error_user);
			exit_flag = 1;
		}
		...
		if(error_group != NULL)
		{
			printf("projmod : Parse error on line %d , Invalid group \"%s\"\n",line_count,error_group);
			exit_flag = 1;
		}		
